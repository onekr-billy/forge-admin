...
18. **【安全】开放网关防重放在验签之前**：`OpenGatewayAuthenticator.authenticateSignature` 顺序 ①assertNotReplayed（写 Redis nonce）②查 client ③凭据校验 ④verifySignature → 无有效凭据的伪造请求可灌 nonce（存储 DoS + nonce 抢占）。应先验签
