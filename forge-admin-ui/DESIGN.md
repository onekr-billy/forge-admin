...
本节以 `/flow/model` 流程模型卡片为基线，适用于流程模型、模板、能力资产、应用入口等“可浏览、可编辑、可发布”的轻量资产列表。目标是企业控制台式紧凑资产卡，不是营销卡片、功能卖点卡或装饰卡片。

### 10.1 视觉基调

- 卡片必须紧凑、克制、可扫读；优先像云控制台资源卡，而不是 AI 生成的彩色卡片墙。
- 使用白色表面、浅灰边框、浅灰 footer 分区和文字链接操作；不要靠大面积色块制造层级。
- 圆角使用小半径：卡片 `2px` 或 `4px`，内部图标容器 `2px`，不要使用 `12px+` 的大圆角。
- 阴影只在 hover 时轻微出现，默认态不应有明显投影。
- 同一张卡片内只允许一个轻量语义图标；图标服务于对象类型识别，不承担装饰职责。
...
```text
Card
├─ Header：左侧对象图标 + 名称 + 稳定编码，右侧状态 Tag
├─ Tags：类型、分类等低频辅助信息
├─ Body：绑定关系 / 简短描述
└─ Footer：左侧元数据，右侧文字操作
```
...
- Header 左侧使用 `32px × 32px` 的轻色图标容器，图标建议使用 `i-lucide:git-merge`、`i-lucide:workflow`、`i-lucide:boxes` 等语义明确的线性图标。
- 名称为主标题，14px、500/600 字重、单行省略；编码使用等宽字体，11px、灰色、单行省略。
- 状态 Tag 放在 Header 右侧，尺寸约 `20px` 高，矩形小圆角；状态文本只能出现一次。若使用组件自身已带状态点，禁止再额外插入圆点。
- Tags 使用灰白小标签，20px 高，最多展示两个高价值标签；不要平铺过多业务标签。
- 绑定关系放在灰底浅边框条中，图标使用链路语义，例如 `i-lucide:link-2`；空绑定使用弱灰色。
- 描述最多一行，缺省文案可以显示“暂无描述”，但不得拉高卡片。
- Footer 使用浅灰背景，左侧为日期和版本，右侧为操作；日期和版本图标建议使用 `i-lucide:calendar`、`i-lucide:git-commit`。
...
| 卡片圆角 | `2px` |
| Header 内边距 | `16px 16px 12px` |
| Body 内边距 | `0 16px 10px` |
| Footer 内边距 | `8px 16px` |
| 图标容器 | `32px × 32px` |
| 标题字号 | `14px` |
| 编码/元数据字号 | `11px` |
...
- Footer 必须左右分区：元数据在左，操作在右。
- 常用操作使用文字链接，不使用小按钮块；链接颜色用主题蓝，hover 只改变颜色。
- 直显操作最多两个，例如“编辑 / 发布”或“编辑 / 实例”；低频操作收入三点菜单。
- 操作之间使用 1px 竖线分隔；不要使用胶囊按钮、描边按钮或彩色按钮组。
- “更多”必须是图标按钮并提供 `aria-label="更多操作"`；图标建议 `i-lucide:more-horizontal`。

### 10.5 禁止项

- 禁止使用 `.card-accent` 这类绝对定位左侧色条、顶部色条或装饰状态线。
- 禁止每张卡片随机分配彩色图标背景；颜色不能成为卡片识别主逻辑。
- 禁止火箭、闪电、星星等营销化图标表示发布、上线、部署；流程资产优先使用发布、分支、合并、版本等工程语义图标。
- 禁止卡片内同时出现大图标块、多个彩色 Tag、彩色边框和明显阴影。
- 禁止把 footer 操作堆在左侧或跟元数据混排；操作必须靠右。
- 禁止为了“高级感”增加渐变、毛玻璃、外发光、浮夸阴影或大圆角。

### 10.6 `/flow/model` 基线

`/flow/model` 已采用本规范作为当前基线。后续同类页面应复用其结构和密度：

- `model-card`：白底、浅边框、小圆角、hover 轻阴影。
- `card-header`：对象身份区 + 状态 Tag。
- `card-body`：类型/分类、业务绑定、描述。
- `card-footer`：左侧 `card-metadata`，右侧 `card-actions`。
- `card-action-link`：文字操作链接。
- `card-more-action`：更多操作图标按钮。
...
- 后台资产卡片使用左侧/顶部装饰色条、随机彩色图标块、火箭发布图标或操作按钮堆叠。
