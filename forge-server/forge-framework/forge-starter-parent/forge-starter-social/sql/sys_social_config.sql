...
CREATE TABLE `sys_social_config` (
  ...
  `client_id` varchar(255) DEFAULT NULL COMMENT '应用ID/Key（旧登录配置字段，兼容期保留；凭据改存应用表后可空）',
  `client_secret` varchar(255) DEFAULT NULL COMMENT '应用Secret（旧登录配置明文字段，兼容期保留；新凭据统一存应用表密文，本列可空）',
