 ...
 */
public record PrintTemplateCreateDTO(@NotNull @Positive Long applicationId, @NotBlank @Pattern(regexp = "[A-Za-z][A-Za-z0-9_-]{0,79}") String templateCode, @NotBlank @Size(max = 100) String templateName, @NotNull PrintSourceType sourceType, @Positive Long pageId, @Size(max = 128) String formKey, @NotBlank @Pattern(regexp = "[A-Za-z][A-Za-z0-9_]{0,99}") String objectCode, @NotBlank @Size(max = DOCUMENT_BYTES) String schemaJson) {
