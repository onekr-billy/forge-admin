<template>
  <NConfigProvider :theme="appStore.isDark ? darkTheme : null" :theme-overrides="appStore.naiveThemeOverrides" :locale="zhCN" :date-locale="dateZhCN">
    <NMessageProvider><NDialogProvider><PreviewContent /></NDialogProvider></NMessageProvider>
  </NConfigProvider>
</template>
...
import FileList from '@/views/system/file-list.vue'
...
const PreviewContent = defineComponent({
  setup() {
    ...
    return () => h('div', { class: 'preview-host' }, [
      ...
      h('main', { class: 'preview-main' }, [h(SystemPageLayout, null, () => h(FileList))]),
    ])
  },
})
</script>
