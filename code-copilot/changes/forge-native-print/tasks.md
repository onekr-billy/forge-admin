...
> 状态：implementing，M1–M5 代码开发及 T46–T49 已完成；用户已选择继续执行 T50–T53，真实业务与物理打印验收仍由用户执行。禁止 push。
>
...
- [ ] T51a：新增 `PAGE_BREAK` 协议与分页游标换页语义，覆盖首尾/连续分页符和最大页数边界。
- [ ] T51b：将设计画布改为多纸张编辑视图，展示页码、自动/手动分页提示、重复页眉页脚和分页估算说明。
- [ ] T51c：执行分页引擎、设计器、Java、构建和浏览器验证后独立 commit，不 push。
- [ ] T52a：实现受信任的代码侧业务打印组件注册器，工厂只展开为标准协议且经过原子校验。
- [ ] T52b：接入业务组件物料分组，并提供审批状态、签章位置、合同条款示例注册项和扩展测试。
- [ ] T52c：执行注册边界、协议导出、设计器和浏览器验证后独立 commit，不 push。
- [ ] T53a：实现校准页生成、浏览器打印能力检查和本地验收清单，复用隔离打印会话。
- [ ] T53b：接入设计器低频入口，覆盖 A3/A4/A5/B4/B5、横纵方向和自定义纸张信息。
- [ ] T53c：执行自动化、构建和 Chromium 浏览器校准页验证；真实 Firefox/Edge/Safari 与物理打印机结果留用户回填，独立 commit、不 push。
