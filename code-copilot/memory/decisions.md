...
缓存管理面只允许按代码注册的应用编码和缓存名查看聚合策略、编辑运行参数、恢复默认和清空整个命名缓存，不提供 Redis 原始 key 枚举、value 读取、任意 key 删除或 pattern 批量清理。旧 Redis 诊断前后端入口和资源权限整体下线，避免共享 Redis 中的会话、凭据及跨模块状态被管理页面暴露或误删。

字典业务读取统一以 `SysDictDataServiceImpl#selectDictDataByType` 的受管注解为唯一缓存事实：前端通用 `useDict` 请求 `/type/{dictType}`，后端字典翻译器每次委托该 Service 代理。翻译器不得再维护独立 TTL Map，否则管理策略、失效通知和运行统计都会被绕过。字典管理页的 `/list` 仅用于管理查询，保留查看停用项和复合条件的能力，不纳入业务读取缓存。

## 74. 审批终态回调与应用业务流程恢复使用提交后边界
