...
**解决方案**：按应用已发布快照的 publishedProcessVersions 和稳定主对象 ID 筛选手动业务流程，外部启动复用 BusinessProcessOrchestrator，输入为已保存记录 ID。固定应用/流程版本与 schemaHash，执行前核验租户、门户、用户委托、节点权限及记录数据范围；不接收任意模型/身份/变量，不在版本漂移时自动换源，也不让外部重放自动重试已有失败运行。旧对象审批明确标注兼容入口；多流程显式选择、单流程自动带入，升级失败重试仍保留原流程编码。
