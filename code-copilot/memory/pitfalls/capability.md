...
> 从 `code-copilot/memory/pitfalls.md` 按主题拆出。新条目追加到本文件。共 22 条。
...
**2026-09-22：单独表单新增的边界补充**：内置表单填报复用页面新增协调器，不应因自动建表使用独立运行库而要求重建表或新建业务动作。单独新增可复用平台已有回执，在调用前提交唯一占位；成功后记 recordId，空占位一律按结果未知拒绝再次新增。此方案是保守的至多一次调用，不代表跨库原子性或自动恢复，仍须人工核对未知结果；上面的建单送审组合 SUBMIT 限制不因此放开。判断占位所有权要比对持久化的候选主键，不依赖受 JDBC useAffectedRows 影响的 INSERT 返回值。
...
**问题描述**：应用发布对象快照遗漏 suiteCode，而 requireSource 依赖该字段强等匹配，导致已发布表单注册失败。只 mock 手工补齐字段的 RuntimeVO 会漏测；null 请求套件还可能与缺字段相等而误通过。

**解决方案**：新快照保存对象自己的 suiteCode；旧快照按当前可信租户、请求套件/编码查未删除对象，以稳定 objectId 和 objectCode 对照不可变发布成员，有套件字段时必须一致。拒绝空参数，不用应用套件或草稿成员兜底，不改历史快照。测试覆盖真实生成/读取链路及跨套件同编码、删除重建、草稿新增边界。

## 应用业务流程开放不能查询旧对象主流程绑定

**发现日期**：2026-09-22

**问题描述**：应用业务流程中的审批节点不等同于旧 ai_business_binding 的 FLOW 主流程；将新应用入口接到旧 FlowActionSourceService，会在应用/流程已发布时仍报“业务对象未发布或未配置启用的主流程”，甚至绕过用户配置的条件、动作与消息节点。

**解决方案**：按应用已发布快照的 publishedProcessVersions 和稳定主对象 ID 筛选手动业务流程，外部启动复用 BusinessProcessOrchestrator，输入为已保存记录 ID。固定应用/流程版本与 schemaHash，执行前核验租户、门户、用户委托、节点权限及记录数据范围；不接收任意模型/身份/变量，不在版本漂移时自动换源，也不让外部重放自动重试已有失败运行。旧对象审批不再向新建用户展示，仅保留存量能力升级兼容；多流程显式选择、单流程自动带入，升级失败重试仍保留原流程编码。

## 系统服务注册来源的风险等级必须与目录发布和授权契约一致

**发现日期**：2026-09-22

**问题描述**：受控系统服务发布器会把注册来源的 `riskLevel` 原样写入目录命令。若新服务标为 `HIGH`，但目录 `publishSystemService` 和客户端授权仅支持 `SYSTEM_SERVICE / ACTION / MEDIUM`，来源、应用和流程全部校验成功后仍会报“受控系统服务能力元数据无效”；只测来源枚举和执行器会漏掉此断链。

**解决方案**：新增系统服务时先对齐“来源元数据 → 发布器命令 → 目录校验 → 客户端授权”四层契约。应用业务流程启动已有固定版本、用户委托、业务权限、数据范围和流程内部审批，按当前协议定义为受控中风险，不为解决注册错误而放宽全局高风险授权。测试同时断言具体服务来源风险等级和发布器最终命令的 `behavior/riskLevel`。

## Jackson JSON null 不能按 Java null 校验系统服务空授权策略

**发现日期**：2026-09-22

**问题描述**：DTO 的 `JsonNode` 字段接收请求体 `null` 时通常得到 Jackson `NullNode`，不是 Java null。只判断 `policy != null` 会把“未配置客户端字段策略”误判为自定义策略，导致受控系统服务授权报“系统服务不接受客户端自定义字段或操作策略”。

**解决方案**：把 Java null、`JsonNode.isNull()` 和空对象统一规范为“无策略”并落库 SQL NULL；非空对象、数组和标量继续拒绝。测试必须覆盖 JSON null，而不能只构造 Java null。

## 能力目录和应用能力页必须共享来源归属并在服务端阻止重复注册

**发现日期**：2026-09-22

**问题描述**：只在应用入口发布时写应用-能力关系，会让从全局目录注册的应用表单/流程能力在应用页不可见，用户误以为未注册并换编码重复创建。仅靠前端列表回填无法防住并发、旧页面或直接 API 调用。

**解决方案**：目录选择应用来源时统一走应用发布接口；对存量目录能力提供显式、幂等的来源同步，严格按应用已发布快照、稳定对象 ID/套件编码和固定流程参数认领，不自动吸附任意 REST。发布前再由服务端按来源身份检查重复，已有同源能力提示复用；列表 GET 保持只读，自动同步失败要可见且可重试。
