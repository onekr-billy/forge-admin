...
with sync_playwright() as p:
    ...
    for oid, name in [('2090956178226143234', '11'), ('2090969102315413505', '12')]:
        ...
        vs = data.get('viewSchema') or {}
        search_fields = [f.get('fieldCode') or f.get('field') for f in ((vs.get('search') or {}).get('fields') or [])]
        list_cols = [c.get('fieldCode') or c.get('field') for c in ((vs.get('list') or {}).get('columns') or [])]
        print(f'object {name}: viewSchema.search =', search_fields)
        print(f'object {name}: viewSchema.list =', list_cols)
    browser.close()
