    ...
    for name in ['测试', '11']:
        ...
        info = page.evaluate('''() => {
            ...
            for (let i = 0; i < all.length; i++) {
                ...
                results.push({
                    previewLiveData: bp.previewLiveData,
                    previewMode: bp.previewMode,
                    designPreview: inst.props?.runtimeCrudProps?.designPreview,
                    draftOnly: inst.props?.runtimeCrudProps?.draftOnly,
                })
                break
            }
            return results
